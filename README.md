...
> 目标：用于合成订阅。一个 mihomo 实例同时开多个本地端口；每个端口可绑定不同订阅与规则；支持保留订阅原始分组，或统一使用 ACL4SSR。

## 目录

- [概念与原理（必读）](docs/01-concepts.md)
- [模式 A：保留订阅原始 group/rules](docs/02-mode-a-preserve.md)
- [模式 B：统一 ACL4SSR（订阅前缀 ns/…）](docs/03-mode-b-acl4ssr.md)
...
- 你只想统一一套分流逻辑，所有端口都按 ACL4SSR → 选 [模式 B](docs/03-mode-b-acl4ssr.md)
